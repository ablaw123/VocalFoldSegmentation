...
"""
Mask R-CNN
Train on the toy VocalFold dataset and implement color splash effect.

Copyright (c) 2018 Matterport, Inc.
Licensed under the MIT License (see LICENSE for details)
Modified by Anthony Law MD, PhD
...
def detect_and_color_splash(model, image_path=None, video_path=None):
    ...
    if image_path:
        ...
        # Return image with mask, score and predictions
        splash = visualize.display_instances(image, r['rois'], r['masks'], r['class_ids'],
                                             class_names, r['scores'], show_mask=False, show_bbox=False, making_image=True)
        # Save output
        # file_name = "splash_{:%Y%m%dT%H%M%S}.png".format(datetime.datetime.now())
        # skimage.io.imsave(file_name, splash)
        file_name = 'splash-boom.png'
    elif video_path:
        video_path2 = os.path.split(video_path)
        video_path3 = video_path2[1]
        video_case = video_path3.rsplit(".", 1)[0]
        ...
        file_name = video_case+"{:%Y%m%dT%H%M%S}.avi".format(datetime.datetime.now())
        ...
        while success:
            with open("/root/glottic_angle.txt", "a") as fout:
                print("frame: ", count)
                # Read next image
                ...
                if success:
                    ...
                    # Return image with mask, score, and angles
                    splash, angles, box_angles = visualize.display_instances(image, r['rois'], r['masks'], r['class_ids'],
                                                                             class_names, r['scores'], show_mask=False, show_bbox=False, colors=colors, making_video=True)
                    # skip images with only one TVF
                    if len(r['class_ids']) > 1:
                        if len(angles) > 1:
                            s = "{} {:10.0f} {:10.1f} {:10.1f} {:10.1f}\n".format(video_case, count, box_angles[0], box_angles[1], (box_angles[0 ] -box_angles[1]))

                            fout.write(s)
                        else:
                            s = "{} {:10.0f} {} {} {}\n".format(video_case, count, '.', '.', '.')
                            fout.write(s)
